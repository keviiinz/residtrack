...
export function CrearTareaForm({ proyectoId }: { proyectoId: number }) {
  const crearTareaConProyecto = crearTarea.bind(null, proyectoId);
  const [state, action, pending] = useActionState(crearTareaConProyecto, undefined);

  return (
    <form
      action={action}
      className="mt-8 space-y-4 rounded-lg border border-zinc-200 p-6 dark:border-zinc-800 dark:bg-zinc-900/40"
    >
      <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-50">Agregar tarea</h2>
      ...
      <div>
        ...
        <input
          ...
          name="titulo"
          type="text"
          className="mt-1 w-full rounded border border-zinc-300 p-2 text-sm dark:border-zinc-700 dark:bg-zinc-950/60 dark:text-zinc-100 dark:focus:border-blue-500 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-blue-500/50"
        />
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          ...
          <input
            ...
            type="date"
            className="mt-1 w-full rounded border border-zinc-300 p-2 text-sm dark:border-zinc-700 dark:bg-zinc-950/60 dark:text-zinc-100 dark:[color-scheme:dark] dark:focus:border-blue-500 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-blue-500/50"
          />
